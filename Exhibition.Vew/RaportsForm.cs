...
//			List<ExhibitionVisitor> visitor_collection = null;
		...
		public RaportsForm(int m, GeneralForm f)
		{
			form = f;
			InitializeComponent();
			progressBar1.Visible = false;
		...
		{
			foreach (var i in view_visitor_collection)
			{
				var pv = context.ExhibitionVisitors.Where(v => v.Email == i.vEmail).Select(s => s).FirstOrDefault();
				form.printVisitor(pv); 
			}
		}
	}
}
